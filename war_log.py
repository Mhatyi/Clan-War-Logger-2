...
def parse_current_state(log):
    ...
    if not log.strip():
        return 127, 1, 5, False  # FIRST RUN
...
if "battles" in current_week_content:
    sorted_battles = sorted(
        current_week_content["battles"], key=lambda x: x[0], reverse=True
    )
    ...
    while i < len(existing_lines):
        ...
        if line.startswith("<details>"):
            ...
            while i < len(existing_lines):
                ...
                if (
                    "Battle Day " in existing_lines[i]
                    and "⚔️" in existing_lines[i]
                ):
                    is_battle = True
                    try:
                        battle_day_num = int(
                            existing_lines[i].split("Battle Day ")[1].split()[0]
                        )
                    except:
                        pass
